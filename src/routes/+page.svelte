...
<div class="max-w-7xl mx-auto px-6 py-10 space-y-10">
  ...
  >
    ...
    <div
      class="flex items-center gap-4 bg-slate-100/50 p-1.5 rounded-2xl border border-slate-200"
    >
      <span
        ...
        <thead>
          ...
          >
            ...
            {#each MESES as mes}
              ...
              >
            {/each}
            <th class="px-8 py-4 border-b border-slate-100 text-right">Total</th
            >
          </tr>
        </thead>
        <tbody class="text-sm font-bold">
          ...
          {#each $dbStore.categorias.filter((c) => c.grupoId === "g1") as cat}
            ...
            {#if showSubcategories}
              {#each $dbStore.subcategorias.filter((s) => s.categoriaId === cat.id) as sub}
                <tr class="bg-slate-50/30 border-b border-slate-50/50 text-xs">
          ...
          <tr class="bg-indigo-600 text-white font-black">
            ...
            <td class="px-8 py-8 text-right text-3xl">
              {MESES.reduce((acc, _, i) => acc + getNetoMes(i), 0).toFixed(2)}€
            </td>
          </tr>
        </tbody>
      </table>
